...
| `grid4.stl` | **Ablage** (komplett geschlossener Einsatz, Einzeldatei) |
| `grid5.stl` | **Becher** (hoher gerundeter Behälter, Voronoi außen, Einzeldatei) |
...
- **Belegung**: `n_bays` (1–4) und `bay1..bay4` (OralB/Sonicare × Ständer/Laden,
  **`tray` = Ablage** (geschlossen) oder **`cup` = Becher** (hoch, gerundet)).
- **Teil-Auswahl** (`part`):
  ...
  - `montage` (Vorschau), `korpus`, `gitter1..4`, `rueckwand`, **`fuss`** (Steck-Fuß),
    **`ablage`** (geschlossener Einsatz), **`becher`** (hoher Becher) – einzelne Teile.
...
openscad -D part_id=4 -o grid4.stl grid.scad       # Ablage (geschlossen)
openscad -D part_id=5 -o grid5.stl grid.scad       # Becher
...
| `bay1..bay4` | s. Belegung | Typ je Fach (`stand_orb`/`charge_orb`/`stand_son`/`charge_son`/`tray`/`cup`) | |
...
| `cup_h` / `cup_wall` / `cup_round` | 80 / 2,5 / 5 | Becher: Höhe / Wandstärke / Rundung | ⟳ |
